...
5. **SSL证书自动续期注意事项（Caddy部署）**：
   - 必须保持服务器安全组80和443端口对外开放，否则Let's Encrypt证书自动续期会失败
   - Caddy会在证书到期前30天自动尝试续期，无需任何手动操作
   - 证书数据存储在`caddy_data`持久化卷中，请勿删除该卷，否则证书会丢失需要重新申请
   - 如果80端口必须关闭，可以改用DNS挑战模式，需要配置阿里云DNS API密钥，详情参考Caddy官方文档
   - 迁移部署时建议备份`caddy_data`卷内容，避免频繁申请证书触发Let's Encrypt速率限制
